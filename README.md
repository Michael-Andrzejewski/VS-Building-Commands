...
`pond_aquarium` is a pond that looks ordinary from the bank and is a three storey aquarium from underneath: 60 across, 25 deep, its whole wall below the waterline glass, with walkway rings 8, 15 and 22 blocks down behind it. Stand where the pier should begin, facing north, and the stone headhouse that appears behind you is the way in. Generated by `tools/gen-pond-aquarium.mjs` and checked by `tools/verify-pond.mjs`.
